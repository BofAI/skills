...
for lock_file in "$ROOT_DIR"/*/package-lock.json; do
  [ -e "$lock_file" ] || continue
  package_file=${lock_file%/package-lock.json}/package.json
  [ -e "$package_file" ] || continue
  lock_versions=$(node -e '
    const data = require(process.argv[1]);
    process.stdout.write([data.version || "", data.packages?.[""]?.version || ""].join("\n"));
  ' "$lock_file")
  lock_version=$(printf '%s\n' "$lock_versions" | sed -n '1p')
  root_package_version=$(printf '%s\n' "$lock_versions" | sed -n '2p')

  relative_path=${lock_file#"$ROOT_DIR"/}
  if [ "$lock_version" != "$release_version" ]; then
    fail "$relative_path has root version ${lock_version:-missing}, expected $release_version"
  fi
  if [ "$root_package_version" != "$release_version" ]; then
    fail "$relative_path has root package version ${root_package_version:-missing}, expected $release_version"
  fi
done
