...
- If API credentials are missing or expired, the wrapper starts API configuration, then continues API collection after configuration succeeds.
...
If the user asks to configure API access, run:
...
If the agent is not inside an interactive Terminal, use the wrapper. It opens a real Terminal window for secure credential input and OAuth callback handling.
