...
如果在 Claude Code 里安装，并希望后续从不同项目目录运行日报时不反复弹 Bash 授权，可以显式安装为：

```bash
python3 twitter-digest/scripts/install.py --client claude --allow-claude-commands --allow-claude-state-read
```

这只会给 Claude Code 增加一条 `run_daily_digest.py` 的全局允许规则，并允许 Read 工具读取 `~/.claude/skills/twitter-digest/.state`。不会放开所有 Bash 命令。也可以不加这个参数，首次运行时在 Claude Code 弹窗里选择允许并“不再询问”。
...
- Agent 正常生成日报时应使用 Read 工具读取 `~/.claude/skills/twitter-digest/.state/run/digest-context.md`。
- Agent 不应使用 `cat`、`head`、`grep`、`python3 -c` 或临时脚本读取 `digest-context.*`，否则 Claude Code 可能在分析阶段再次弹 Bash 授权。
...
```bash
rm -rf ~/.claude/skills/twitter-digest/.state/chrome-profile
...
```bash
python3 ~/.claude/skills/twitter-digest/scripts/compare_collectors.py --rounds 1 --interval-sec 120
```
