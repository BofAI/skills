...
3. If no usable token exists, it starts API configuration in a real Terminal when necessary.
4. It runs `api_x_digest.py`.
5. It builds current-run context files with `digest_context.py`.
...
- Missing config: run `run_daily_digest.py --configure-api`.
