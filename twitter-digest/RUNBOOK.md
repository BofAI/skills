...
推荐路径是 OAuth1 PIN，适合用户自己申请的本地 X Developer App：

1. 脚本弹出 OAuth / Paste Token 选择。
2. 用户选择 OAuth1 PIN。
3. 脚本提示输入 X App 的 API Key / Consumer Key。
4. 脚本提示输入 X App 的 API Key Secret / Consumer Secret。
5. 脚本打开 X 授权页。
6. 用户在浏览器里授权 app。
7. X 显示 PIN，用户复制并粘贴回脚本提示框。
8. 脚本换取 user access token 和 access token secret。
...
1. 用户选择 OAuth2。
2. 脚本提示输入 X Developer App Client ID。
3. 脚本打开 X 授权页。
4. 用户在浏览器里登录/授权。
5. 脚本通过本地 callback 收到授权码，换取 user-context access token。
6. token 保存到 `twitter-digest/.state/api_config.json`，文件权限尽量设为 owner-only。
7. 后续日报 `--source auto` 自动走 API；token 快过期时自动 refresh。
