...
```bash
TMPDIR="$(mktemp -d)" && git clone --depth 1 --branch v1.5.11-beta.6 https://github.com/BofAI/skills.git "$TMPDIR/skills" && python3 "$TMPDIR/skills/twitter-digest/scripts/install.py" --client codex
```
...
```bash
TMPDIR="$(mktemp -d)" && git clone --depth 1 --branch v1.5.11-beta.6 https://github.com/BofAI/skills.git "$TMPDIR/skills" && python3 "$TMPDIR/skills/twitter-digest/scripts/install.py" --client claude --allow-claude-commands --allow-claude-state-read
```
