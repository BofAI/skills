...
After a standard install, the installed unified configuration wizard immediately checks X API and X Chat. Existing valid configuration is reused; missing or invalid configuration is completed in the same Terminal flow. Custom `--skills-dir` installs and dry runs skip this step. To install without starting configuration, set `TWITTER_DIGEST_CONFIGURE_AFTER_INSTALL=0`.
