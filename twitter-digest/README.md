...
```bash
curl -fsSL https://raw.githubusercontent.com/BofAI/skills/v1.5.13-beta.1/twitter-digest/install.sh | env TWITTER_DIGEST_INSTALL_CLIENT=codex sh
```

Claude Code:

```bash
curl -fsSL https://raw.githubusercontent.com/BofAI/skills/v1.5.13-beta.1/twitter-digest/install.sh | env TWITTER_DIGEST_INSTALL_CLIENT=claude TWITTER_DIGEST_ALLOW_CLAUDE_COMMANDS=1 TWITTER_DIGEST_ALLOW_CLAUDE_STATE_READ=1 sh
```
...
`run_daily_digest.py` uses API. If API credentials are missing or expired, the wrapper starts API configuration and then continues API collection.
...
The setup flow uses OAuth2 PKCE. It asks for the X Developer App Client ID, opens the X authorization page, waits for the local callback, and saves a user-context access token plus refresh token.
