...
- 默认每个等我回复会话最多向上滚动 40 次，并最多保留 300 条消息气泡。
- 日报目标是近 24 小时；如果已加载消息的时间戳显示已经超过 24 小时窗口，会提前停止上滚。
- 消息统计是“本次浏览器加载到的消息”，不是完整历史归档。
- `digest-context.md` 会为需要总结的等我回复会话输出 `DM Thread Context`，最多带最近 300 条已加载消息，并保留 raw label、URL、加载状态等原始信息，方便模型理解复杂上下文。
- 默认只打开今天等我回复的会话。
