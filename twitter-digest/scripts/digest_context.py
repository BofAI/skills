...
def build_digest_facts(data: dict[str, Any], summary: dict[str, Any]) -> dict[str, Any]:
    ...
    if (summary.get("dm_status") or "") in {"blocked_by_x_chat_passcode", "dm_page_loading_timeout", "visible_threads_unopened", "no_visible_threads", "api_dm_unavailable", "api_dm_error", "api_dm_todo"}:
        facts["data_gaps"].append(
            {
                "source": "messages",
                "status": summary.get("dm_status"),
