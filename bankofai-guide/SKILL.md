...
2. Run `agent-wallet --version` to check if the CLI is installed.
   - **Not installed** → inform the user that Agent Wallet CLI is required for wallet features, and ask if they want to install it now.
   - **User confirms** → ask whether to install stable or beta version:
     - **stable** → run `npm install -g @bankofai/agent-wallet`
     - **beta** → run `npm install -g @bankofai/agent-wallet@beta`
   - **User declines** → acknowledge and end.
...
5. Show the generated password to the user. Inform them that the password has been auto-saved to `~/.agent-wallet/runtime_secrets.json`, but they should also memorize or securely store the password — if runtime secrets are deleted, the password is needed to restore access.
